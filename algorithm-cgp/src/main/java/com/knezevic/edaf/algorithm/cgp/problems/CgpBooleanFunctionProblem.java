...
/**
 * Boolean function problem for CGP (simplified, no external dependencies).
 * Tries to find a boolean function that outputs 1 for as many inputs as possible.
 */
public class CgpBooleanFunctionProblem implements CgpProblem {
    private final int n;
    private final List<Function> functionSet;

    public CgpBooleanFunctionProblem(Map<String, Object> params) {
        this.n = 4; // 4 variables
        ...
        this.functionSet = List.of(
            ...
            new Function("NOR", 2, args -> (!(args[0] >= 0.5 || args[1] >= 0.5)) ? 1.0 : 0.0)
        );
    }
    ...
    public void evaluate(CgpIndividual individual) {
        ...
        // Count number of inputs that produce output 1
        int ones = 0;
        for (int i = 0; i < (1 << n); i++) {
